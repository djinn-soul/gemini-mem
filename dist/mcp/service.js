...
exports.MemoryMcpService = void 0;
...
function readInt(input, key, fallback, min, max) {
    const value = input[key];
    if (value === undefined || value === null) {
        return fallback;
    }
...
function stripPrivateSegments(text) {
...
class MemoryMcpService {
    resolveScope(input) {
        ...
        const projectId = explicitProjectId ||
            (0, project_id_1.resolveProjectId)({
                cwd,
        ...
        return {
            cwd,
            projectId,
            dbPath: (0, db_path_1.resolveMemoryDbPath)(config.dbPath, cwd)
        };
    }
    memoryStatus(input) {
        const scope = this.resolveScope(input);
        const store = new store_1.SqliteMemoryStore(scope.dbPath);
        try {
            return {
    ...
    memorySaveObservation(input) {
        ...
        const card = (0, schema_1.parseMemoryCard)({
            title,
            type,
            summary,
        ...
        const dedupeHash = (0, dedupe_1.buildDedupeHash)(scope.projectId, redactedCard);
        const store = new store_1.SqliteMemoryStore(scope.dbPath);
        try {
...
exports.MemoryMcpService = MemoryMcpService;
